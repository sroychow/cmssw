...
void DetidTreeAnalyzer::analyze(const edm::Event& iEvent, edm::EventSetup const& es) {
  detVec_->clear();
  //For Digi
  ...
  es.get<TrackerDigiGeometryRecord>().get(geom_handle);
  ...
  for (it = input->begin(); it != input->end(); ++it) {
    ...
    DetInfo dinfo;
    const GeomDetUnit* gDetUnit = tGeom->idToDetUnit(DetId(detId_raw));
    ...
    dinfo.isbarrel = pxb;
    if(pxb) {
      PixelBarrelName pbname(DetId(detId_raw), tTopo, true);
      dinfo.layer = pbname.layerName();
      dinfo.shell = pbname.shell();
    }
    else { 
      PixelEndcapName pename(DetId(detId_raw), tTopo, true);
