...
void SiPixelValidateTrackFromSoA::analyze(const edm::Event& iEvent, edm::EventSetup const& es) {
  ...
  //originally taken from RecoPixelVertexing/PixelTrackFitting/plugins/PixelTrackProducerFromSoA.cc, but before Patatrack PR merge
  ...
  for (int32_t it = 0; it < soatracks.stride(); ++it) {
    ...
    nTk++;
    auto q = quality[it];
    if (q != pixelTrack::Quality::loose)  continue;
    nTkLoose++;
  }
