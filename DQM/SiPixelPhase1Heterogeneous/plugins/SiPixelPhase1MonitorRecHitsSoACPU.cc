...
SiPixelPhase1MonitorRecHitsSoACPU::SiPixelPhase1MonitorRecHitsSoACPU(const edm::ParameterSet& iConfig) : SiPixelPhase1MonitorRecHitsSoABase(iConfig) {
...
void SiPixelPhase1MonitorRecHitsSoACPU::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (!rhsoaHandle.isValid()) {
    edm::LogWarning("SiPixelPhase1MonitorRecHitsSoACPU") << "No RecHits SoA found \n returning!" << std::endl;
    return;
  }
  ...
  edm::LogWarning("SiPixelPhase1MonitorRecHitsSoACPU") << "Total RecHits : " << nHits_ << std::endl; 
  hnHits->Fill(nHits_);
  ...
  for(uint32_t i=0;i<nHits_;i++){
    ...
    int16_t sizeX=soa2d->clusterSizeX(i);
    int16_t sizeY=soa2d->clusterSizeY(i);
